...
# dimension reduction of numerical simulation data.
...
def init_parser():
    ...
    # define our own version of the slypi parser
    ...
    parser.add_argument("--xy-header", help="Root name of header for (x,y) coordinates " 
                        "columns in .csv file.")
    parser.add_argument("--xy-ps-tag", action="store_true", help="Add [XY Pair] tag to (x,y) "
                        "header output for parameter space models.")
    ...
    parser.add_argument("--file-batch-size", type=int, help="Train reduction model "
                        ...
                        "all algorithms, see slypi.ensemble.algorithms.reduction --help for options.")
...
def main(arg_list=None):
    ...
    log = logging.getLogger("slypi.ensemble.reduce")
    ...
    plugin, arg_list = ensemble.plugin(args.plugin, arg_list)
    ...
    ensemble_table = EnsembleTable(log, csv_file=args.csv_file,
    ...
    if args.xy_out is not None:
        ...
        # output coordinates
            ...
            # pre-pend ps tags, if requested
            if args.xy_ps_tag:
                x_header = "[XYpair X]" + x_header
                y_header = "[XYpair Y]" + y_header
