...
# in the dac generic format and/or pushed directly to a Slycat server.
...
def catalog_tdms_files (arguments, log):
    ...
    metadata = tdms_util.catalog_tdms_files(arguments, log)
...
def read_tdms_files(arguments, metadata, run_chart_matches, common_tdms_types, log):
    ...
    for row in range(len(metadata)):
        ...
        for tdms_file_ind in range(len(metadata[row]["tdms_files"])):
            ...
            # read tdms file
            tdms_file_info = None
            if arguments.input_tdms_zip:
                
                # read zipped tdms file
                archive = metadata[row]["source"].split("!")
                zip_file = archive[0]
                file_in_zip = '/'.join([archive[1], tdms_file_name])
                with ZipFile(zip_file, 'r') as zip_ref:
                    with zip_ref.open(file_in_zip) as file_in_zip:
                        with nptdms.TdmsFile.open(file_in_zip) as tdms_file:

                            # get tdms file information
                            tdms_file_info = parse_tdms_file (arguments, 
                                                metadata[row]["source"],
                                                run_chart_matches,
                                                tdms_file_path, 
                                                tdms_file_name, 
            ...
            # record tdms file information
            if tdms_file_info:

                # track file info
                root_properties.append(tdms_file_info["root_property"])
                run_charts.append(tdms_file_info["run_chart"])
                run_chart_headers.append(tdms_file_info["run_chart_header"])
                shot_numbers.append(tdms_file_info["shot_number"])
                module_ID.append(tdms_file_info["module_ID"])
                module_SN.append(tdms_file_info["module_SN"])

                # do we keep this row?
                if tdms_file_info["skipped_row"]:
                    skipped_rows[row] = 1

            else:
                # this should never happen, 
                # any error should have already been
                # captured in parse_tdms_file
                raise TDMSUploadError('Error reading tdms file: "' +
                                      tdms_file_name + '".')
...
# parse tdms file, using tdms file object
def parse_tdms_file (arguments, source, run_chart_matches, 
                     tdms_file_path, tdms_file_name, tdms_file,
                     log):
    
    # save the root properties for the .tdms file
    root_properties = pd.DataFrame(tdms_file.properties, index=[0])
    ...
    if rc_matches == []:
        ...
        run_charts = numeric_group_df[header_matches]

        # also save run chart header matches
        run_chart_headers = rc_matches

        # also save shot numbers
        try:
            shot_numbers = group_df["Shot Number"]
        except KeyError:
            if arguments.use_group_numbers:
                shot_numbers = group_nums
            else:
                raise TDMSUploadError('Missing shot number in "' +
                    source + '", try --use-group-numbers to ' +
                    'substitute value with group name.')

        # check for module SN
        module_IDs = check_module_col(group_df, "Module ID", tdms_file_path, log)
        module_SNs = check_module_col(group_df, "Module SN", tdms_file_path, log)

    return {'root_property': root_properties, 
            'run_chart': run_charts, 
            'shot_number': shot_numbers, 
            'module_ID': module_IDs,
            'module_SN': module_SNs,
            'run_chart_header': run_chart_headers,
...
def create_model(arguments, log):
    ...
    # check that we have tdms support
    if not nptdms:
        raise ImportError("The tdms module has not been installed, use " + 
                         "pip install slypi[tdms].")
    ...
    # if using input-tmds-zip, make sure zip file is supplied
    if arguments.input_tdms_zip:
        if not arguments.input_tdms_zip[0].endswith(".zip"):
            raise TDMSUploadError ('--input-tdms-zip used without .zip file.')
